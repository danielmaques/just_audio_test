...
class DecriptedAudioSource extends StreamAudioSource {
  ...
  Future<HttpClientResponse> _fetch() async {
    ...
    subscription = response.listen((data) async {
      int newProgress = _progress + data.length;
      if (newProgress < 20496 && _decrypted == false) {
        _decryptBuffer.addAll(data);
        _progress += data.length;
        return;
      } else if (newProgress >= 20496 && _decrypted == false) {
        _decryptBuffer.addAll(data);
        _progress += data.length;
        Uint8List decryptedBuffer =
            decryptAESCryptoJS(_decryptBuffer.sublist(0, 20496), key);
        List<int> partialBuffer =
            _decryptBuffer.sublist(20496, _decryptBuffer.length);
        List<int> finalBuffer = [];
        finalBuffer.addAll(decryptedBuffer);
        finalBuffer.addAll(partialBuffer);
        data = finalBuffer;
        _decryptBuffer = [];
        _decrypted = true;
      } else {
      ...
      //data = handleData(data, _progress);
      //_progress += data.length;
...
List<int> handleData(List<int> data, int progress) {
  return data;
}
